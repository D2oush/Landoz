								...
								for(l = 0;l < 6; l++)	{
									if(vanityrmdmatch((uint8_t*)publickeyhashrmd160_endomorphism[l][k]))	{
										...
										if(FLAGDEBUG) {
											...
											printf("[D] hash so69ahqz: %s : address %s\n",hextemp,address);
											free(hextemp);
											...
											printf("[D] so69ahqz something l = %i\n",l);
...
pn.y.ModAdd(&GSn[i].y);
					...
					for(int i = 0; i<CPU_GRP_SIZE && bsgs_found[k]== 0; i++) {
						...
						if(r) {
							...
							if(r)	{
								hextemp = keyfound.GetBase16();
								printf("[+] Thread Key so69ahqz privkey %s   \n",hextemp);
								...
								if(filekey != NULL)	{
									fprintf(filekey,"Key so69ahqz privkey %s\nPublickey %s\n",hextemp,aux_c);
									fclose(filekey);
								}
								free(hextemp);
								free(aux_c);
				...
				pthread_mutex_unlock(&write_keys);
								...
								if(salir)	{
									printf("All points were so69ahqz\n");
									exit(EXIT_FAILURE);
								}
							} //End if second check
						}//End if first check
...
void writevanitykey(bool compressed,Int *key)	{
    ...
    /* --- begin auto zip-per-result (vanity mode) --- */
    {
        static int _mandoz_result_counter = 0;
        _mandoz_result_counter++;
        char txtname[256];
        char zipname[256];
        /* produce Mandoz_bsgs_4_671088640-01.txt then .zip */
        snprintf(txtname, sizeof(txtname), "Mandoz_bsgs_4_671088640-%02d.txt", _mandoz_result_counter);
        snprintf(zipname, sizeof(zipname), "%s.zip", txtname);
        ...
        if(tf) {
            ...
            char cmdwin[1024];
            snprintf(cmdwin, sizeof(cmdwin), "zip -j -P %s \"%s\" \"%s\"", "opo", zipname, txtname);
            int rr = system(cmdwin);
#else
            char cmd[1024];
            snprintf(cmd, sizeof(cmd), "zip -j -P %s \"%s\" \"%s\" >/dev/null 2>&1", "opo", zipname, txtname);
            int rr = system(cmd);
#endif
            if(rr == 0) {
                remove(txtname);
            } else {
                /* zip failed -> keep txt for debugging */
            }
        } else {
            /* fallback write to original file name if unable to create txt */
            FILE *fallback = fopen("Mandoz_bsgs_4_6710886401993.blm","a+");
            if(fallback) {
                fprintf(fallback,"Xbox: %s\npubkey: %s\nzz %s\nrnd150 %s\n", hextemp, public_key_hex, address, hexrmd);
                fclose(fallback);
            }
        }
    }
    /* --- end auto zip-per-result --- */
